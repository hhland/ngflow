...
function LoadMenusOnEmpChange() {
    ...
    if (selectIndex >= 0) {
        $("#pageloading").show();
        Application.data.getEmpOfMenusByEmpNo(options[selectIndex].value, 0, "false", showMenusTree, this);
    }
}
//查询
function QueryEmps() {
    ...
    $("#pageloading").show();
    Application.data.getEmpsByNoOrName(objSearch, function (js, scope) {
        if (js == "") js = "[]";
        var pushData = eval('(' + js + ')');
