...
function LoadMenusOnStationChange() {
    ...
    if (selectIndex >= 0) {
        $("#pageloading").show();
        Application.data.getStationOfMenusByNo(options[selectIndex].value, 0, 'false', LoadMenuTreeCallBack, this);
    }
}
//加载菜单
...
function QueryStation() {
    ...
    $("#pageloading").show();
    Application.data.getStationByName(objSearch, function (js, scope) {
        if (js == "") js = "[]";
        var pushData = eval('(' + js + ')');
        var objSelect = document.getElementById("lbStation");
        if (pushData.length > 0) {
