...
function RefMethodExt(  index , warning, target, ensName, key , path)
{
 ...
 else
 {
  ...
  var url= "/Comm/RefMethod.aspx?Index="+index+"&EnsName="+ ensName +"&"+ key;
 // alert (url);
    if (target==null)
      var a=window.location.href=url; 
    else
...
function EditDtl( dtlName, refKey)
{
 if (CurrEnsName==null)
      return ;
   var newWindow=window.open( '/Comm/UIEnDtl.aspx?EnsName='+dtlName+"&RefKey="+refKey+"&MainEnsName="+ CurrEnsName +CurrKeys,'chosecol', 'width=700,top=100,left=200,height=400,scrollbars=yes,resizable=yes,toolbar=false,location=false' );
   newWindow.focus();
   return true;
}
...
function EditDtl1(path, ensName , dtlName, refKey, enKeys)
{
 if (ensName==null)
      return ;
   var newWindow=window.open('/Comm/UIEnDtl.aspx?EnsName='+dtlName+"&RefKey="+refKey+"&MainEnsName="+ ensName +enKeys,'chosecol', 'width=700,top=100,left=200,height=400,scrollbars=yes,resizable=yes,toolbar=false,location=false' );
   newWindow.focus();
   return true;
}
