...
function onSelect(note) {
    if (note.data.MenuType == 4) {
        ...
        if (note.data.IsStart == "1") {
            f_addTab(fk_flow, " I initiated " + note.data.Name + " Historical Process ", "StartHistory.aspx?FK_Flow=" + fk_flow + "&Name=" + note.data.Name);
            return;
        }
        else if (note.data.IsStart == "0") {
            f_addTab(fk_flow, " I participated in the " + note.data.Name + " Historical Process ", "../WF/Rpt/Search.aspx?DoType=My&FK_Flow=" + fk_flow);
            return;
        }
    }
}