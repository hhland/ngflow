...
field_title_Name = field_title_Name || "Name";
field_title_BatchLaunched = field_title_BatchLaunched || "Batch launched";
field_title_FlowChart = field_title_FlowChart || "Flow chart";
field_title_HistoryLaunched = field_title_HistoryLaunched || "History launched";
field_title_Description = field_title_Description || "Description";
grid_title = grid_title || "The process can be initiated";

link_BatchLaunched = link_BatchLaunched || "Batch launched ";
link_TurnOn = link_TurnOn || "Turn on";
link_CheckOut = link_CheckOut || "Check out";
...
function LoadGrid() {
    var grid = $('#maingrid').datagrid({
        ...
        columns: [[
                    {
                        field: 'Name', title: field_title_Name, width: 380, align: 'left', formatter: function (value, rec) {
                        var h = "";
                            var rname = rec.NAME.replace(" ","_");
                        if (rec.STARTLISTURL) {
                        ...
                        return "<a href='javascript:void(0);' onclick=ShowEasyUiTitleDiv('" + rec.NO + "','" + rname + "','" + h + "')>" + rec.NAME + "</a>";
                    }
                    },
                    {
                        field: 'IsBatchStart', title: field_title_BatchLaunched, formatter: function (value, rec) {
                        var h = "";
                        if (rec.ISBATCHSTART == "1") {
                            h = "../WF/BatchStart.aspx?FK_Flow=" + rec.NO;
                            h = "<a href='javascript:void(0);' onclick=StartListUrl('" + h + "')> " + link_BatchLaunched + " </a>";
                        }
                        return h;
                    }
                    },
                    {
                        field: 'RoleType', title: field_title_FlowChart, formatter: function (value, rec) {
                            var rname = rec.NAME.replace(" ", "_");
                        return "<a href='javascript:void(0);' onclick=OpenEasyUiFlowPicture('" + rec.NO + "','" + rname + "')> "+link_TurnOn+" </a>";
                    }
                    },
                    {
                        field: 'HistoryFlow', title: field_title_HistoryLaunched, width: 180, formatter: function (value, rec) {
                            var rname = rec.NAME.replace(" ", "_");
                        return "<a href='javascript:void(0);' onclick=ShowEasyUiHistoryData('" + rec.NO + "','" + rname + "')> "+link_CheckOut+" </a>";
                    }
                    },
                    {
                        field: 'Note', title: field_title_Description, width: 280, formatter: function (value, rec) {
