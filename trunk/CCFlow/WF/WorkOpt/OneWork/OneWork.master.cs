    ...
    public partial class WF_WorkOpt_OneWork : BP.Web.MasterPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (OneWorkXml item in xmls)
            {
                ...
                string iname = GetGlobalResourceObject("Menu", item.No+".Text").ToString();
                Pub1.AddLi(string.Format("<div{2}><a href='{0}'><span class='nav'>{1}</span></a></div>{3}", url, 
                    iname, 
                    item.No == pageId ? " style='font-weight:bold'" : "", Environment.NewLine));
            }
