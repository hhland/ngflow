...
public class Handler : IHttpHandler, IRequiresSessionState  
{
    ...
    string webUserNo {
        get {
            string val = request.Params["WebUserNo"];
    ...
    string webUserName
    {
        get
        {
            string val = request.Params["WebUserName"];
            if(string.IsNullOrWhiteSpace(val))
                val = WebUser.Name;
    ...
    string webUserFK_Dept
    {
        get
        {
            string val = request.Params["WebUserFK_Dept"];
            if(string.IsNullOrWhiteSpace(val))
                val = WebUser.FK_Dept;
    ...
    public string DealSQL(string sql, string key)
    {
        ...
        sql = sql.Replace("@WebUser.No", webUserNo);
        sql = sql.Replace("@WebUser.Name", webUserName);
        sql = sql.Replace("@WebUser.FK_Dept", webUserFK_Dept);
        if (oid != null)
    ...
    public void ProcessRequest(HttpContext context)
    {
       ...
       if(string.IsNullOrWhiteSpace(WebUser.No))
            Dev2Interface.Port_Login(no);
       ...
       // key = "周";
        switch (me.ExtType)
        {
            //case BP.Sys.MapExtXmlList.DDLFullCtrl: //  Cascaded ddl.
            //    sql = this.DealSQL(me.DocOfSQLDeal, key);
            //    dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
            //    context.Response.Write(JSONTODT(dt));
            //    return;
            case BP.Sys.MapExtXmlList.ActiveDDL: //  Dynamic filling ddl.
                sql = this.DealSQL(me.DocOfSQLDeal, key);
                dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
                context.Response.Write(JSONTODT(dt));
                return;
            case BP.Sys.MapExtXmlList.TBFullCtrl: //  AutoComplete .
            case BP.Sys.MapExtXmlList.DDLFullCtrl: //  Cascaded ddl.
                switch (context.Request.QueryString["DoType"])
                {
                    case "ReqCtrl":
                        //  Get padding  ctrl  Information value .
                        sql = this.DealSQL(me.DocOfSQLDeal, key);
                    ...
                    case "ReqM2MFullList":
                        ...
                        break;
                    case "ReqDtlFullList":
                        /*  Get filled collection from the table . */
                        ...
                        break;
                    case "ReqDDLFullList":
                        /*  To get a personalized populated drop-down box . */
                    ...
                    case "ReqDDLFullListDB":
                        /*  To obtain the value of personalized populated drop-down box .  According to have been passed over  ddl id. */
   ...
   if ((BP.Sys.SystemConfig.AppCenterDBType == DBType.Informix
        ...
        {
            /* If the database is not case sensitive ,  We must follow the user input sql Secondary treatment .*/
