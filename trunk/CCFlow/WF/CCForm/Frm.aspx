...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        function queryForBool(sql) {
            var url = "/WF/CCForm/Frm.ashx?action=queryForBool";
            var param = {sql:sql};
        ...
        function BindingForm(formid) {
            var frameid = "F"+formid;
            return window.parent.document.getElementById(frameid).contentWindow;
        }
        ...
        var isChange = false;
        function SaveDtlData(isForceSave) {
            // Plus regular verification 
            isForceSave = isForceSave || false;
            if (SysCheckFrm() == false)
                return false;

            if (isChange == false&&!isForceSave)
                return;
            ...
            var btn = document.getElementById("<%=Btn_Save.ClientID %>");
            if (btn) {
                if (window["kindeditors"]) {
                    var kes = window["kindeditors"];
                    for (var key in kes) {
                        kes[key].sync();
                    }
                }
        ...
        function SaveM2MVal(){
           ...
           for(var i=0;i<frames.length;i++){
               var frame=frames[i];
               if(frame.contentWindow.SaveM2M){
                  frame.contentWindow.SaveM2M(true);
               }
           }
        }
