...
function ReturnValTBFullCtrl(ctrl, fk_mapExt) {
    ...
    if (!row) return;
    ctrl.value = row["No"];
    var namePrefix = ctrl.name.substring(0, ctrl.name.lastIndexOf("$"));
    for (var key in row) {
        var elname = namePrefix + "$TB_" + key;
        var ddlname = namePrefix + "$DDL_" + key;
        //var el = document.getElementsByName(elname);
        //if (el) {
        //alert(elname+":"+row[key]);
        var fieldval = row[key], fieldname = key;
        //var oid = key.substring(key.lastIndexOf("_")+1);
        $("[name='" + elname + "'],[name='" + ddlname + "']").val(fieldval);
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            ...
            for (var i in dataObj.Head) {
                if (typeof (i) == "function")
                    continue;
                if (dataObj.Head[i]["No"] != e) continue;
                for (var k in dataObj.Head[i]) {
                    if (k == 'No' || k == 'Name')
                        continue;

                    //  alert(k + ' val= ' + dataObj.Head[i][k]);
                    var el = $("#" + beforeID + 'TB_' + k + "," + "#" + beforeID + 'TB_' + k + endId + "," + "#" + beforeID + 'DDL_' + k + "," + "#" + beforeID + 'DDL_' + k + endId);
                    ...
                    // var oid = endId.substring(1);
                    // updateDtl(oid, k, dataObj.Head[i][k]);
                }
            }
        },
        complete: function (XMLHttpRequest, textStatus) {
