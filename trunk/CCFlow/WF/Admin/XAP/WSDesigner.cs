    ...
    public class WSDesigner : System.Web.Services.WebService
    {
            ...
            try
            {
                ...
                switch (doType)
                {
                    ...
                    case "DelFrm":
                        ...
                        md4.Delete();
                        //delete m2m row
                        string sql = string.Format("delete from sys_mapm2m where FK_MapData='{0}' ",v1);
                        DBAccess.RunSQL(sql);
                        sql = string.Format("delete from Sys_MapData where No='{0}'", v1);
                        DBAccess.RunSQL(sql);
                        return null;
        ...
        public DataSet GetWorkList(string fk_flow, string workid)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                sql = "SELECT NDFrom, NDTo,ActionType,Msg,RDT,EmpFrom,EmpFromT FROM " + table + " WHERE WorkID=" + workid + "  OR FID=" + workid + " ORDER BY RDT ASC, NDFrom ASC,NDTo ASC";
