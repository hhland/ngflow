<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MyFlow_ModifyOldDataCol.ascx.cs" Inherits="CCFlow.WF.MyFlow_ModifyOldDataCol" %>
...
<div style="margin: 0; padding: 0;" id="D">
    ...
    <asp:HiddenField ID="hifOldWorkID" runat="server" Value="0" />
    <asp:Button ID="btnSubmit" runat="server" Text="Button" style="display:none;" />
</div>

<script type="text/javascript">
    document.onreadystatechange = function () {
        if ($("#<%= hifOldWorkID.ClientID %>").val() == "0") {
            var wName;
            var RadNum = Math.random();
            wName = window.showModalDialog('MyFlow_ModifyOldDataSelect.aspx?Radstr=' + RadNum + '&FK_Flow=' + "<%= FK_Flow %>" + "&FK_Node=" + "<%=FK_Node %>", '', 'dialogWidth:1135px;DialogHeight=718px;status:no;help:no;resizable:yes;');
            if (wName == null) { }
            else {
                $("#<%= hifOldWorkID.ClientID %>").val(wName);
