...
public partial class WF_MapDef_UC_MExt : BP.Web.UC.UCBase3
{
    ...
    public int BindRegularExpressionEditExt(int idx, string myEvent)
    {
        ...
        string FK_MapData = Request.Params["FK_MapData"],
            ...
            ;
        string src = "/WF/MapDef/FrmSelfjsTab.aspx?MyPK=" +me.MyPK + "&FK_MapData=" + FK_MapData + "&event=" + myEvent + "&OperAttrKey=" + OperAttrKey + "&docid=" + tbdocid
            , title = FK_MapData + ":" + OperAttrKey + ":" + myEvent;
        //hl.NavigateUrl = "javascript:EUIWinOpen('"+src+"','"+title+"').window('maximize');";
        hl.Attributes["onclick"] = "javascript:EUIWinOpen('" + src + "','" + title + "').window('maximize');";
        hl.Style["color"]="blue";
        hl.Style["cursor"] = "pointer";
        hl.Style["text-decoration"] = "underline";
        this.Pub2.AddTD(hl);
