...
namespace BP.En
{
    ...
    abstract public class EnObj
    {
        ...
        public void RemoveValByKey(string attrKey)
        {
            this.Row.Remove(attrKey);
        }

        public bool ContainsKey(string attrKey)
        {
           return this.Row.ContainsKey(attrKey);
        }
        ...
        public int GetValIntByKey(string key)
        {
            try
            {
                return Int32.Parse(this.GetValStrByKey(key));
            }
            catch (Exception ex)
            {
