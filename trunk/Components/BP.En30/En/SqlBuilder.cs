...
namespace BP.En
{
    ...
    {
        #region  With respect to IEntitiy Operations 
        ...
        {
            //  Judgment special circumstances .
            switch (en.PKField)
            {
                case "OID":
                    return " OID=@OID";
            ...
            if (en.EnMap.Attrs.Contains("OID"))
            {
                ...
                if (key == 0)
                    throw new Exception("@ In execution [" + en.EnMap.EnDesc + " " + en.EnMap.PhysicsTable + "], Did not give PK OID  Assignment , Query operation can not be performed .");
                //   if (en.PKs.Length == 1)
                return " OID=@OID" + key;
            }
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                if (attr.MyFieldType == FieldType.PK || attr.MyFieldType == FieldType.PKFK || attr.MyFieldType == FieldType.PKEnum)
                {
                    if (attr.MyDataType == DataType.AppString)
                    {
                        ...
                        if (val == null || val == "")
                            throw new Exception("@ In execution [" + en.EnMap.EnDesc + " " + en.EnMap.PhysicsTable + "] Did not give PK " + attr.Key + "  Assignment , Query operation can not be performed .");
        ...
        public static String GetKeyConditionOfOLE(Entity en)
        {
            ...
            //				if (key==0)
            //					throw new Exception("@ In execution ["+en.EnMap.EnDesc+ " " +en.EnMap.PhysicsTable +"], Did not give PK MID  Assignment , Query operation can not be performed .");
        ...
        {
            //  You can not delete the physical table name , Cause undefined column .
        ...
        public static string Retrieve(Entity en)
        {
            ...
            switch (en.EnMap.EnDBUrl.DBType)
            {
                ...
                case DBType.Informix:
                    ...
                    break;
                case DBType.DB2:
                    throw new Exception(" Has not been achieved .");
                default:
                    throw new Exception(" Has not been achieved .");
            }
        ...
        public static string GenerFormOfOra(Entity en)
        {
            ...
            from += ",";
            //  Generate foreign key table list .
            ...
            // Generate a list of enumeration table .
        ...
        public static string GenerFormWhereOfOra(Entity en)
        {
            ...
            //  Began to form   Foreign key  where.
            foreach (Attr attr in fkAttrs)
            {
                if (attr.MyFieldType == FieldType.RefText)
                    continue;
        ...
        public static string GenerCreateTableSQLOfMS(Entity en)
        {
            ...
            if (attrs.Count == 0)
                throw new Exception("@" + en.EnDesc + " ,  There is no set of attributes  attrs.Count = 0 , Create data tables to perform .");
            ...
            foreach (Attr attr in attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppInt:
                        if (attr.IsPK)
                        {
                            // Description This is the automatic growth of the columns .
                            if (attr.UIBindKey == "1")
        ...
        public static string GenerCreateTableSQLOfOra_OK(Entity en)
        {
            if (en.EnMap.PhysicsTable == null)
                throw new Exception(" You do not have to [" + en.EnDesc + "], Set physical table .");
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@ No defined data types ! attr=" + attr.Key + " MyDataType =" + attr.MyDataType);
                }
            }
        ...
        /// <summary>
        ///  Set up SelectSQLOfOLE 
        /// </summary>
        /// <param name="en"> To be executed en</param>
        /// <param name="topNum"> The highest number of inquiries </param>
        /// <returns> Return query sql</returns>
...
#warning 20111-12-03  There should not be an exception .
...
#warning 2011-12-03  There should not be an exception .
                            if (attr.UIBindKey.Contains("."))
                                throw new Exception("@" + en.ToString() + " &UIBindKey=" + attr.UIBindKey + " @Key=" + attr.Key);
        ...
        {
            //  Determine whether the memory inside  此sql.
            string sql = "";
        ...
        public static string UpdateOfMSAccess(Entity en, string[] keys)
        {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppBoolean:
                        val = val + ",[" + attr.Field + "]=" + en.GetValStringByKey(attr.Key);
                        break;
                    case DataType.AppDate: //  If the date type .
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@SqlBulider.update,  Without this type of data ");
                }
            }
            ...
            if (val == "")
            {
                /* If you do not appear to be updated .*/
                foreach (Attr attr in en.EnMap.Attrs)
                {
                    if (keys != null)
                    {
                ...
                //return null;
                //throw new Exception(" There has been an unreasonable update : There is no data to be updated .");
            }
            ...
            if (val == "")
            {
                ...
                throw new Exception(en.EnDesc + " Perform the update error : Invalid property [" + ms + "] For this entity for .");
            }
        ...
        /// <summary>
        ///  Produce to be updated sql  Statement 
        /// </summary>
        /// <param name="en"> To update entity</param>
        /// <param name="keys"> To update keys</param>
        /// <returns>sql</returns>
        public static string Update(Entity en, string[] keys)
        {
            Map map = en.EnMap;
            if (map.Attrs.Count == 0)
                throw new Exception("@ Entity :" + en.ToString() + " ,Attrs Properties collection of information loss , Failed to generate SQL.");
            ...
            foreach (Attr attr in map.Attrs)
            {
                ...
                /* There may be a case where two of the primary key .*/
                //  if (attr.IsPK)
                //  continue;
            ...
            foreach (Attr attr in map.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppBoolean:
                        ...
                        break;
                    case DataType.AppInt:
                        if (attr.Key == "MyPK") // Special judge resolved truck 是64 Bits int Types of numerical problems .
        ...
        public static string UpdateForPara(Entity en, string[] keys)
        {
            ...
            if (string.IsNullOrEmpty(val))
            {
                ...
                //   throw new Exception("@ Generate SQL Error :" + map.EnDesc + "," + en.ToString() + ", To update the field is empty .");
            }
            if (!string.IsNullOrEmpty(val))
                val = val.Substring(1);
            string sql = "";
