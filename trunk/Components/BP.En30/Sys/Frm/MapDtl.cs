...
namespace BP.Sys
{
    ...
    public class MapDtl : EntityNoName
    {
        ...
        public static string GenTableHtml(string EnsName,long workid) {
            return new MapDtl(EnsName).genTableHtml(workid);
        }

        public string genTableHtml(long workid)
        {
            string html = "";
            try
            {
                MapAttrs attrs = new MapAttrs(this.No);
                string dtlsql = string.Format("select * from {0} where REFPK='{1}'", this.No, workid);
                DataTable dt = DBAccess.RunSQLReturnTable(dtlsql);
                int numOfCol = 0;
                 html = " <table border='1' > ";
                html += "<thead><tr>";
                ...
                foreach (MapAttr attr in attrs)
                {
                    if (attr.UIVisible == false || attr.IsPK || attr.KeyOfEn == "OID" || (this.IsEnableGroupField && this.GroupField == attr.KeyOfEn))
                        continue;
                    ...
                    foreach (MapAttr attr in attrs)
                    {
                        ...
                        switch (attr.MyDataType)
                        {
                            ...
                            case DataType.AppRate:
                                try
                                {
                                    tr += string.Format("<td>{0}</td>", decimal.Parse(val).ToString("0.00"));
                                }
                                catch (Exception ex)
                                {
                                    tr += string.Format("<td>{0}</td>", "0.00");
                                }
                                break;
                            default:
                                tr += string.Format("<td>{0}</td>", val);
                ...
                html = string.Format("<fieldset><legend>dtl {0} draw error</legend>{1}</fieldset>",this.No,ex.Message);
            }
            return html;
        }
