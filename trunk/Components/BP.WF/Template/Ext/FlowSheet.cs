    ...
    public class FlowSheet : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddBoolean("Use_OldWorkID", false, "If apply Updating Workflow (Yes/No)?", true, true, true);

                map.AddTBStringDoc("OldWorkID_SQL", null, "Updating Workflow ID selection SQL Script", true, false, true);
        ...
        public string DoDelDataOne(int workid, string note)
        {
            ...
            if (workid == 0)
            {
                return " Can not delete workid=" + workid + ";";
            }
            else
            {
