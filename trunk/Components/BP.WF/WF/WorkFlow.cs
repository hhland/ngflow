...
namespace BP.WF
{
    ...
    public class WorkFlow
    {
        ...
        public static void DeleteFlowByReal(string flowNo, Int64 workid, bool isDelSubFlow)
        {
            ...
            if (DBAccess.RunSQLReturnValInt(string.Format("SELECT use_oldworkid FROM WF_Flow where no={0}", flowNo)) == 0)
            {
            ...
            foreach (Node nd in nds)
            {
                try
                {
                    DBAccess.RunSQL("DELETE FROM " + nd.PTable + " WHERE OID=" + workid + " OR FID=" + workid);
                }
                catch (Exception ex)
                {
                    msg += "@ delete data error " + ex.Message;
        ...
        public string DoFlowOver(ActionType at, string stopMsg, Node currNode, GERpt rpt)
        {
            ...
            // Detail data processing copy Issue .  First check : The current node £¨ The last node £© Is there a list .
