...
namespace SMSServices
{
    ...
    public partial class FrmMain : Form
    {
        ...
        private void Btn_StartStop_Click(object sender, EventArgs e)
        {
            ...
            if (this.Btn_StartStop.Text == " Start up ")
            {
                if (this.thread == null)
                {
                    ...
                    thread.Start();
                    new Thread(new ThreadStart(RunItMsg)).Start();
        ...
        private void DoSendMsg()
        {
            ...
            foreach (SMS sm in sms)
            {
                ...
                if (string.IsNullOrWhiteSpace(sm.Email) &&string.IsNullOrWhiteSpace(sm.SendTo))
                {
        ...
        public void SendMail(SMS sms)
        {
            ...
            if (string.IsNullOrEmpty(sms.Email))
            {
               // BP.WF.Port.WFEmp emp = new BP.WF.Port.WFEmp(sms.SendTo);
                string empemail = DBAccess.RunSQLReturnString(string.Format("select email from port_emp where no='{0}'", sms.SendTo));
                sms.Email = empemail;
            }
    ...
    { return true; };
            ...
            string fromaddr = SystemConfig.GetValByKey("SendEmailAddress", "ccflow.cn@gmail.com")
                , pwd = SystemConfig.GetValByKey("SendEmailPass", "ccflow123")
                , user = fromaddr.Split('@')[0]
                ;


            System.Net.Mail.MailMessage myEmail = new System.Net.Mail.MailMessage();
            myEmail.From = new MailAddress(fromaddr, "ccflow", System.Text.Encoding.UTF8);

            myEmail.To.Add(sms.Email);
            myEmail.Subject = sms.Title;
            myEmail.SubjectEncoding = System.Text.Encoding.UTF8;// Mail header encoding 

            string docEmail = sms.DocOfEmail;
            docEmail = docEmail.Replace("\r\n", "<br/>").Replace("\n", "<br/>").Replace("\r", "<br/>");
            myEmail.Body = docEmail;
            ...
            try
            {
                ...
                client.SendCompleted += (e, args) => {
                    
                    sms.HisEmaiSta = MsgSta.RunOK;
                    if (args.Error != null) {
                        sms.HisEmaiSta = MsgSta.RunError;
                        sms.MsgType = args.Error.Message;
                    }
                    sms.Update();
