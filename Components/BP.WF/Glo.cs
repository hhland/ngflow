                ...
                else
                {
                    ...
                    catch (Exception ex)
                    {
                        if (BP.WF.Glo.OSModel == WF.OSModel.WorkFlow)
                            throw new Exception("@  Carried out sql Failure : There may be reasons , You installed on GPM, Also installed ccflow, But web.config The model is not BPM Mode , You need to web.config的OSModel=1");
                        throw ex;
                    }
                    sqlscript = BP.Sys.SystemConfig.CCFlowAppPath + "\\WF\\Data\\Install\\SQLScript\\Port_Inc_CH_WorkFlow.sql";
                    BP.DA.DBAccess.RunSQLScript(sqlscript);
        ...
        public static Attrs AttrsOfRpt
        {
            get
            {
                if (_AttrsOfRpt == null)
                {
                    ...
                    _AttrsOfRpt.AddTBDecimal(GERptAttr.FlowDaySpan, 0, " Span (天)", true, false);
