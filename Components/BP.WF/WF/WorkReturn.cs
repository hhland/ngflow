    ...
    public class WorkReturn
    {
        ...
        private void DeleteSpanNodesGenerWorkerListData()
        {
            ...
            if (this.ReurnToNode.IsStartNode == true)
            {
                ...
                ps.Clear();
                ps.SQL = "DELETE FROM WF_GenerWorkerList WHERE FK_Node!=" + this.ReurnToNode.NodeID + " AND (WorkID=" + dbStr + "WorkID1 OR FID=" + dbStr + "WorkID2)";
...
#warning  Did not consider two processes data storage mode .
            ...
            if (Glo.IsEnableSysMessage == true)
            {
                ...
                string title = string.Format(" Return to work : Process :{0}. The work :{1}, Return man :{2}, You need to deal with ",
